...
You are working on **nano-vwap** — a Spring Boot WebSocket app that connects to the Alpaca real-time trade feed and calculates VWAP (Volume Weighted Average Price).
...
**Non-obvious:**
- Auth flow is ordered: on WS connect, send `AuthMessage` first, then `SubscribeMessage` with symbols — order matters
- `ALPACA_API_KEY` and `ALPACA_API_SECRET` must be set as env vars; they are injected via `${...}` placeholders in `application.properties` — do not hardcode them
